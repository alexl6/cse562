...
private let initialThres: Float = 0.6
...
class SquareCameraViewController: UIViewController {
    ...
    private var confidenceThres: Float = initialThres
    ...
    private let confidenceLabel: UILabel = {
        ...
        label.numberOfLines = 0
        label.translatesAutoresizingMaskIntoConstraints = false
        label.text = String(format: "Confidence Threshold: %.2f", initialThres)
        label.textAlignment = .center
        label.font = UIFont.systemFont(ofSize: 14)
        return label
    }()
    ...
    private func setupUI() {
        ...
        NSLayoutConstraint.activate([
            ...
            confidenceLabel.topAnchor.constraint(equalTo: previewView.bottomAnchor, constant: 20),
            ...
            confidenceSlider.topAnchor.constraint(equalTo: confidenceLabel.bottomAnchor, constant: 10),
            confidenceSlider.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            confidenceSlider.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.7),
    ...
    private func setupYOLOModel() {
        do {
            ...
            } else if let packageURL = Bundle.main.url(forResource: "yolov8n", withExtension: "mlpackage") {
    ...
    private func processDetectionResults(request: VNRequest) {
        ...
        guard let results = request.results as? [VNRecognizedObjectObservation] else {
            print("Bad result type from VNRequest")
            return;
        }
        
        // Note to self: use filter instead of for loop in the future
        // https://developer.apple.com/documentation/swift/defaultindices/filter(_:)
        let filteredResults: [VNRecognizedObjectObservation] = results.filter{$0.confidence >= confidenceThres}
    ...
    @objc private func sliderValueChanged(_ sender: UISlider) {
        let step: Float = 0.01
        let roundedThres = round(sender.value / step) * step
        // Update slider & label to rounded value
        sender.value = roundedThres
        confidenceThres = roundedThres
        self.confidenceLabel.text = String(format: "Confidence Threshold: %.2f", confidenceThres)
    }
